...
### 4.3 平台卡数要求 (基于 MXFP4→BF16 反量化约束)

**关键前提**: HuggingFace 发布的 Kimi-K3 权重为 MXFP4 + BF16 混合精度。A2/A3 不支持 MXFP4 原生计算，加载 Expert 权重时必须反量化 MXFP4→BF16 (4×膨胀):
- Expert 权重 (MXFP4): 1300 GB → BF16: 5200 GB
- 非 Expert (BF16): 260 GB → 维持: 260 GB
- 全 BF16 总权重: ~5460 GB

LoRA 微调下单卡显存 (base 冻结, 仅 LoRA adapter 有优化器):

```
Expert 权重 (BF16):   5200/E  GB    ← 冻结, EP 分片
非 Expert 权重 (BF16): 260/N  GB    ← 冻结, FSDP2 分片
激活值 (重计算+卸载后): ~15   GB
LoRA 优化器:           ~3    GB
...
| 128 | 128 | 7 | 60.6 GB | ✅ |
| 256 | 128 | 7 | 59.6 GB | ✅ |
| 256 | 256 | 3.5 | 39.3 GB | ✅ 充裕 |

**LoRA 微调最少需要**:
- A2: **128 卡** (EP=128), 推荐 256 卡
- A3: **128 卡** (EP=128), 推荐 256 卡
- A5: **16 卡** (EP=8, MXFP4 原生支持, 无需反量化)
